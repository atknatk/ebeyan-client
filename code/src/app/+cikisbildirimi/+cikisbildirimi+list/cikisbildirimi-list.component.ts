...
export class CikisbildirimiListComponent implements OnInit, AfterViewInit {
    ...
    $('#ozetbeyan-table').on('click', 'tr', function () {
      ...
      if ($(this).hasClass('row-selected')) {
        ...
        if (ozby) {
          ...
          if (ozby.cikisBildirimiIslemCagirHataList && ozby.cikisBildirimiIslemCagirHataList.length > 0) {
  ...
  private edit(ozbyRow: any) {
    let ths = this;
    if (ozbyRow) {
      ths.router.navigate(['/cikisbildirimi/cikisbildirimi', ozbyRow.id]);
    } else {
  ...
  private getOptions() {
    let ths = this;
    return {
      restUrl: 'cikisbildirimi',
      editForm: null,
      title: 'Çıkış Bildirimi Listesi',
      ...
      datatableOptions: new DnDatatableBase({
        ...
        deleteButton: false,
        refreshButton: true,
        restUrl: 'cikisbildirimi/simplepaging'
      }),
    };
  }
  ...
  private hatalar(ozbyRow: any) {
    let ths = this;
    if (ozbyRow) {
      this.hataList = ozbyRow.cikisBildirimiIslemCagirHataList;
  ...
  private imzalaVeTescilEt(ozbyRow: any) {
    if (isNullOrUndefined(ozbyRow.tescilNo)) {
      this.tescilService.tescilCikisBildirimi(ozbyRow);
    } else {

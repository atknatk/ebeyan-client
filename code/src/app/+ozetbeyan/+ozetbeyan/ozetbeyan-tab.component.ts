...
@Component({
  templateUrl: './ozetbeyan-tab.component.html',
  styles: [`
    .li-disabled {
      /* pointer-events: none;*/
      /* opacity: 0.6;*/
...
export class OzetbeyanTabComponent extends DnLoadingBase implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    this.sub = this._route.params.subscribe(params => {
      ...
      this.ozetbeyanComponent.beyanType = beyanConfig.ozetbeyan;
      this.ozetbeyanComponent.loadData(this.idOzetbeyan);
      if (this.tasimaSenetComponent) {
        this.tasimaSenetComponent.beyanType = beyanConfig.ozetbeyan;
  ...
  saveOzetbeyanEvent(data: any) {
    if (this.tasimaSenetComponent) {
      if (data.beyanTuru) {
        this.beyanTuruCode = data.beyanTuru.id;
      }
      this.tasimaSenetComponent.beyanType = beyanConfig.ozetbeyan;

...
export class VarisbildirimiListComponent implements OnInit, AfterViewInit {
    ...
    $('#ozetbeyan-table').on('click', 'tr', function () {
      ...
      if ($(this).hasClass('row-selected')) {
        ...
        if (ozby) {
          ...
          if (ozby.varisBildirimiIslemCagirHataList && ozby.varisBildirimiIslemCagirHataList.length > 0) {
  ...
  private edit(ozbyRow: any) {
    let ths = this;
    if (ozbyRow) {
      ths.router.navigate(['/varisbildirimi/varisbildirimi', ozbyRow.id]);
    } else {
  ...
  private getOptions() {
    let ths = this;
    return {
      restUrl: 'varisbildirimi',
      editForm: null,
      title: 'Varış Bildirimi Listesi',
      ...
      datatableOptions: new DnDatatableBase({
        ...
        deleteButton: false,
        refreshButton: true,
        restUrl: 'varisbildirimi/simplepaging'
      }),
    };
  }
  ...
  private hatalar(ozbyRow: any) {
    if (ozbyRow) {
      this.hataList = ozbyRow.varisBildirimiIslemCagirHataList;
  ...
  private imzalaVeTescilEt(ozbyRow: any) {
    if (isNullOrUndefined(ozbyRow.tescilNo)) {
      this.tescilService.tescilVarisBildirimi(ozbyRow);
    } else {
